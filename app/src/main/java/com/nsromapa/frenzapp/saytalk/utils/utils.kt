...
import com.nsromapa.frenzapp.newfy.ui.views.FrenzAppVideoView
...
object utils {
    ...
    object constants {
        ...
        const val URI_AUTHORITY = "com.nsromapa.saytoonz.frenzapp.provider"
        ...
        const val APP_SHORT_LINK = "https://frenzapp.nsromapa.com"
//        const val APP_LINK = "https://play.google.com/store/apps/details?id=${BuildConfig.APPLICATION_ID}"
        const val APP_LINK = "https://frenzapp.nsromapa.com"
    ...
    fun getFormattedTenDigitNumber(number:String ) : String {
        val regex = "[^0-9]"
//        var out = Pattern.compile(regex1).matcher(number).replaceAll("")
        var out = number.replace(Regex("[()\\-\\s]"), "")

        if(out.length>9){
            out = "+233"+number.substring(number.length - 9)
        }else{
...
//          val intent = Intent(context, FrenzAppVideoView::class.java)
...
//          context.startActivity(intent)
    ...
    fun getNameFromNumber(context: Context, number: String):String{
        ...
        try {
            ...
            if(number == getFormattedTenDigitNumber(FirebaseUtils.getPhoneNumber())
                    ...
                    ) || FirebaseUtils.getPhoneNumber().contains(
                            getFormattedTenDigitNumber(
                                    number
                            )
                    )) {
    ...
    public fun shareInviteText(context: Context){
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "text/plain"
            putExtra(Intent.EXTRA_TEXT,
                utils.constants.SHARING_TEXT
            )
            putExtra(Intent.EXTRA_SUBJECT, "FrenzApp")
        }
