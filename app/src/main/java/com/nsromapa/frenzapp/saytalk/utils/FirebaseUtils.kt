...
//                                                MemoryPolicy.NO_STORE)
    ...
    fun getUid(): String = FirebaseAuth.getInstance().uid.toString()

    fun getPhoneNumber(): String = FirebaseAuth.getInstance().currentUser!!.phoneNumber!!
    ...
    fun checkForUpdate(context: Context, shouldShowToast: Boolean) {
        ...
        val date_app_added = "dateAdded"
        val app_download_url = "downloadLink"
        val what_is_new = "what_is_new"

        //this will return an int
        FirebaseDatabase.getInstance().reference.child("AppUpdateInfo")
                .addListenerForSingleValueEvent(object : ValueEventListener {
                    override fun onCancelled(p0: DatabaseError) {
                    }
                    ...
                    override fun onDataChange(p0: DataSnapshot) {
                        ...
                        val versionCode = p0.child(key_app_code).getValue(Int::class.java)
                                ?: BuildConfig.VERSION_CODE
                        val downloadLink = p0.child(app_download_url).getValue().toString()
                        val appDated = p0.child(date_app_added).getValue().toString()
                        val newUpdates = p0.child(what_is_new).getValue().toString()
                        Log.d(
                                "FirebaseUtils",
                        ...
                        if (versionCode > com.nsromapa.frenzapp.BuildConfig.VERSION_CODE) {
                            //show update dialog
                            context.alert {
                                positiveButton("Get Update") {
                                    context.browse(downloadLink)
                                }
                                negativeButton("Cancel") {
                                }
                                title = "Update available"
                                message = "A New Update is available for FrenzApp\n\nWhat is New\n$newUpdates"
                                isCancelable = false
                            }
                                    .show()
    ...
    fun removeMemberFromChannel(uid: String, channelID: String,
                                phoneNumber: String, channelName: String,
                                isRemovedByOther: Boolean) {
        if (isRemovedByOther)
    ...
    fun showTargetOptionMenuFromProfile(context: Context, uid: String,
                                        groupID: String, phoneNumber: String, isAdmin: Boolean,
                                        isMeAdmin: Boolean, groupMembers: MutableList<Models.GroupMember>,
                                        groupName: String) {
...
///////////////////////////////////////////////////////////////////
    ...
    fun showTargetOptionMenuFromProfileForChannel(context: Context, uid: String,
                                                  channelID: String, phoneNumber: String, isAdmin: Boolean,
                                                  isMeAdmin: Boolean, channelMembers: MutableList<Models.ChannelMember>,
                                                  channelName: String) {
        context.selector(
                "", listOf(
                "${if (isAdmin) "Dismiss" else "Make"} Admin",
