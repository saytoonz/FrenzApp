...
import com.nsromapa.frenzapp.saytalk.libs.ImagePickActivity
import com.nsromapa.frenzapp.saytalk.libs.VideoPickActivity
            ...
            override fun onBindViewHolder(
                ...
                if (getItemViewType(position) == TYPE_EVENT) {
                    ...
                    if (position > 0) {
                        ...
                        textHolder.dateTextView.visibility = if (!DateFormatter.isSameDay(Date(model.timeInMillis), previousDate)) {
                            View.VISIBLE
                        } else {
                            View.GONE
                        }
                ...
                targetSenderTitle?.let {
                    if (isGroup) {
                        ...
                        if (channelMembers.isNotEmpty())
                            try {
                                FirebaseUtils.ref.user(channelMembers.filter { it.uid == model.from }[0].uid)
...
//                        container.addView(unreadView)
                ...
                if (position > 0) {
                    ...
                    dateHeader.visibility = if (!DateFormatter.isSameDay(date, previousDate)) {
                        View.VISIBLE
                    } else {
                        View.GONE
                    }
...
//            .child(utils.constants.FILE_TYPE_VIDEO).child(messageID)
    ...
    private fun setMyVideoHolder(holder: Holders.MyVideoMsgHolder,
                                 model: Models.MessageModel,
                                 messageID: String) {
        ...
        holder.cardContainer.setCornerEnabled(true,
                true, model.caption.isEmpty(),
                false)
                        ...
                        when (p1?.itemId) {
                            ...
                            R.id.action_translate -> {
                                ...
                                if (Pref.getSettingFile(context).contains(
                                    ...
                                    alert {
                                        message = "You have not set any default language. Go to Menu -> Setting to set it."
    ...
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        ...
        ////Hiding all video calls and audio call buttons..
        videoCallItem?.isVisible = false
        audioCallItem?.isVisible = false
                ...
                .addValueEventListener(object : ValueEventListener {
                    ...
                    override fun onDataChange(p0: DataSnapshot) {
                        ...
                        p0.children.forEachIndexed { index, dataSnapshot ->
                            ...
                            message?.let {
                                val textMessage = message.message.takeIf { message.messageType == "message" }
                                        ?: message.messageType

                                if (it.from == targetUid && !TextUtils.isEmpty(it.from) && !TextUtils.isEmpty(targetUid))
