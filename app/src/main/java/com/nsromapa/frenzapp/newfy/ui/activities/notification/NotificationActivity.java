    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mFirestore.collection("Users")
                .document(FirebaseAuth.getInstance().getCurrentUser()
                        .getUid()).get().addOnSuccessListener(documentSnapshot -> {
    ...
    private void updateReadStatus() {
        ...
        if(read.equals("false")){
            ...
            mFirestore.collection("Users").document(current_id).collection("Notifications")
                    .document(getIntent().getStringExtra("doc_id")).update(readMap).addOnSuccessListener(aVoid -> Log.i("done","read:true")).addOnFailureListener(new OnFailureListener() {
                @Override
        ...
        mSend.setOnClickListener(view -> {
            ...
            if (!TextUtils.isEmpty(message_)) {
                ...
                mFirestore.collection("Users/" + user_id + "/Notifications_reply").add(notificationMessage).addOnSuccessListener(documentReference -> {
                    ...
                    Toasty.success(NotificationActivity.this, "Reply sent!", Toasty.LENGTH_SHORT,true).show();
                    message.setText("");
